...
def establish_connection():
	...
	print "Enter your exponent_client"
	global base
	global prime
	global key
	global inVect
	...
	sendVal = send(server, str(dhkeyPublic) )
	if(sendVal):
		print 'Send successful'
		receiveVal = receive_blocking(server)
		print 'Received value is ', receiveVal
		dhkeyPrivate = pow(int(receiveVal), exponent)%prime
...
def send_string(server_con, input_str):			#takes string and connection from the application layer and 
	global key
	global inVect
	cipherText = encryption(input_str, key, inVect)
	cipherT = list2str(cipherText)
	cipherT_auth = return_digest(cipherT)
	packet = cipherT + cipherT_auth
	success = send(server_con, packet)
	return success			
	
def receive_string(server_con):			#looks if there is something in the received buffer, if no, do nothing
	cipherText = receive(server_con)
	global key
	global inVect				# if yes, send the decrypted text back to application layer
	if cipherText==-1 or cipherText==None:
		return -1
	else:
		if return_digest(cipherText[0:len(cipherText)-32]) == cipherText[ len(cipherText) - 32: len(cipherText)] :
			strg = cipherText[0:len(cipherText)-32]
			cipherT = cipherText[0:len(cipherText)-32].split(",")
			print cipherT
