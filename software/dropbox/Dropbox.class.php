...
class Dropbox
{
	...
	private static $APP_KEY 		= 'zecyufc5vktalt5';
	private static $APP_SECRET		= 'hwj8aa2oling7no';
	private static $CALLBACK_URL 		= 'http://localhost/cs455/software/dropbox/example.php';
	...
	function getMeta($root, $path) {
		return $this->get('https://api.dropbox.com/1/metadata/'.$root.'/'.$path);	
	} 
	
	function getFolder($root, $path) {
	    $meta = json_decode($this->getMeta('dropbox', NULL));
	    ...
	    foreach($content as $c) {	    
	      $cc = (array) $c;
	      $arr[] = array("path" => $cc['path'], "is_dir" => $cc['is_dir']);
  	    }
	    return $arr;
	}
	...
	function getHTML($arr) {
		...
		//$s .=  $nl;
		if($a['is_dir'] == 1) {
		  $s .=  "<tr> <td><a href = ''>".$a['path']." ".$a['is_dir']. "</a> </td> </tr>";
		} else { 
		  $s .= "<tr> <td>". $a['path']." ".$a['is_dir']. " </td> </tr>";
		}			
	    }	
